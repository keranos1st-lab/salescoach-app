...
manager_notes — «Разбор по менеджерам»:
  ЗАПРЕЩЕНО упоминать: company_profile, skill_breakdown, risk_id,
  manager_risk_actions, excluded, negatives[], positives[].
  Если данных по анкете компании мало — пиши:
  «данные по продукту заполнены частично, выводы по УТП делаем осторожно»
  НЕ называй это company_profile.
  ...
  Писать как живой коуч, не как система.

coaching_focus — «План действий на следующую неделю»:
  Ровно 5 пунктов. Первые 2 — самое срочное (горит). Последние 2 — системное (на перспективу).
  coaching_focus — СТРОГОЕ ПРАВИЛО:
  Все 5 пунктов ОБЯЗАНЫ быть разными по смыслу и формулировке.
  КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: повторять одну и ту же структуру
  «разобрать X звонков на встрече 1:1 и зафиксировать скрипт».
  Каждый пункт = одно конкретное действие с глаголом в начале.
  Пункты должны быть разноплановыми — например:
    пункт 1: конкретный разбор звонков (что и с кем слушать)
    пункт 2: ролевая игра или отработка навыка
    пункт 3: постановка измеримого KPI
    пункт 4: системное действие (скрипт, чеклист, шаблон)
    пункт 5: контроль — как проверить результат через неделю
  Максимум 1 пункт про «разобрать звонки», остальные — про другое.
  Никаких внутренних технических терминов в тексте (company_profile, skill_breakdown,
  risk_id и подобных). Писать как живой коуч, не как система.
